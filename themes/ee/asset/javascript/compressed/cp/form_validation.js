...
markFieldInvalid:function(t,i){this._toggleErrorForFields(t,i)},/**
	 ...
	 */
_toggleErrorForFields:function(i,e){var s=i.parents("form"),a=i.parents("div[class*=setting]").not("div[class=setting-note]"),n=a.parents("fieldset").size()>0?a.parents("fieldset"):a.parent(),r="em.ee-form-error-message",o=!1,d=i.parents(".tab"),l=d.size()>0?d.attr("class").match(/t-\d+/):"",// Grabs the tab identifier (ex: t-2)
u=t(d).parents(".tab-wrap").find('a[rel="'+l+'"]'),// Tab link
...
f=c?d.find(".form-ctrls .btn"):s.find(".form-ctrls .btn");
...
n.hasClass("grid-publish")&&(n=n.find("div.setting-txt"),a=i.parents("td"),o=!0),"success"==e)
...
o?(a.removeClass("invalid"),
// For Grid, only remove the invalid class from the label if no
// more errors exist in the Grid
0==n.parent().find("td.invalid").size()&&n.removeClass("invalid")):n.removeClass("invalid"),a.find("> "+r).remove(),
// If no more errors on this tab, remove invalid class from tab
u.size()>0&&!this._errorsExist(d)&&u.removeClass("invalid"),
// Re-enable submit button only if all errors are gone
(!this._errorsExist(s)||!this._errorsExist(d)&&c)&&(f.removeClass("disable").removeAttr("disabled"),f.each(function(i,e){e=t(e),e.is("input")?e.attr("value",e.data("submit-text")):e.is("button")&&e.text(e.data("submit-text"))}));else{
