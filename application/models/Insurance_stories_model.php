...
public function get_stories_all(){
	$query=  $this->db->select('*')
					...
					->where(array('kct.channel_id'	=>	INSURANCE_STORIES_CHANNEL_ID, 
								  'kct.site_id'		=>	SITE_ID,
								  'kct.status'		=>	'open'))
								  ->order_by("kcd.field_id_409", "desc")
					->get();
...
function getarticles(){
	...
	public function get_episodes($entryId,$get_episodes) {
		...
		return  $this->db->select('col_id_55 as image, col_id_52 as title, col_id_54 as description, col_id_53 as short_description,col_id_58 as lines')
	...
	function get_previous_record($field_id_464){
	$previous_record_query = $this->db->select('*')
	->from('kotak_channel_titles as kct')
	->join('kotak_channel_data as kcd', 'kcd.entry_id = kct.entry_id')
	->where(array(
		'kct.channel_id' => INSURANCE_STORIES_CHANNEL_ID,
		'kct.site_id' => SITE_ID,
		'kct.status' => 'open',
		'kcd.field_id_409 <' => $field_id_464 // Assuming $current_record->field_id_464 holds the value of field_id_464
	))
	->order_by("kcd.field_id_409", "desc")
	->limit(1)
	->get();
	return $previous_record = $previous_record_query->row();
	}
	
// Get the next record
function get_next_record($field_id_464){
$next_record_query = $this->db->select('*')
	->from('kotak_channel_titles as kct')
	->join('kotak_channel_data as kcd', 'kcd.entry_id = kct.entry_id')
	->where(array(
		'kct.channel_id' => INSURANCE_STORIES_CHANNEL_ID,
	...
	->limit(1)
	->get();
return $next_record = $next_record_query->row();
}
}
?>