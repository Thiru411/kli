...
<style>
    /*page 1 css v*/
    .custom-pagination-new{text-align: center;}
    .custom-pagination-new .Products{display: inline-block !important; float: none;}
    .prod-img1 {
        width: 100%;
    }
    .watch:hover {
    ...
    .show_more.Products1{height: 45px !important;}
    .show_more.Products1:hover{ color: #fff !important;}
...
width:40px;
        ...
        .kotak-tv-banner {top:0 !important; margin-top: 0 !important;}
 .product-tab {top:0; padding-bottom: 4rem;
    }
        ...
        <div class="dd-fflex align-baseline">
                ...
                <div class="tabs-btns mx-675" id="btnid">
                    ...
                    if($categoryDetails){
                        foreach($categoryDetails as $cat_info){
                            ...
                            ?>
                    <button class="btncls categaory_name <?php if($i==1){echo "active";}?> activeclass<?=$i?>"  onclick="openProduct('<?=$cat_info->category_name?>',<?=$i?>)"><?=$cat_info->category_name?><?=$live_img?></button>    
                        <?php $i++;}}?>
                </div>
            </div>
            <div class="w-30 text-right select-lan">
                <div class="position-relative">
                    ...
                    <select onchange="get_language(this)" name="language" id="language">
...
function get_language(val){
	var language = val.value;
	var catname='';
  $(val).closest(".select-lan").prev().find("#btnid").children().each(function () {
    ...
    var paginations= 1;
                $.ajax({
                        url: '<?=base_url()?>site/product_detailsByLimit111',
                        type: 'POST',
                        data: { paginations:paginations,catname:catname,language:language },
                        success: function(response){
                            var resp1 = $.parseJSON(response);
                            if(resp1.status==1){
                            $('.sssssss'+catname).html(resp1.product_details)
							
                            }else{
							$('.sssssss'+catname).html('')
							}
                            if(resp1.nextrec=='0'){
                                $('.ww'+catname).addClass('d-none')
                            }
                           // var resp1 = $.parseJSON(response);
...
<?php if($info->category_name=="Live"){?><h1 class="live-text">Live Now</h1><?php }                     
    $j=1;
?>
            <div class="dd-fflex sssssss<?=$info->category_name?>">
            <?php 
		...
		$n=1;
		 foreach($productDetails as $pinfo)
		 {
            ...
            <div class="<?=$info->category_name?>2 dd-fflex ">
                        </div>    
                   
</div>
    
<?php if($n>12){?>
   <div class="custom-pagination custom-pagination-new www<?=$info->category_name?>"  >
    <input type="hidden" id="paginations<?=$k?>" name="pagination" value="2">
                 <a  class="link show_more Products1 <?=$info->category_name?>"  onclick="showMore('<?=$k?>','<?=$info->category_name?>')" style="padding:12px 20px;text-transform:uppercase;color:#da251c;font-size:12px;border:1px solid #da251c;border-radius:50px;margin:0 10px 0 0;text-align:center;height:50px;line-height:revert; display: table;text-decoration:none; " >load more</a>
              </div>
             <?php }?>   
...
<script>
    ...
    // $('.show_more').click(function() {
        function showMore(i,catname){
        var paginations=$('#paginations'+i).val();
        var language=$('#language').val();
        $('#paginations'+i).val(parseInt(paginations)+parseInt(1));
        
                $.ajax({
                        url: '<?=base_url()?>site/details_products',
                        type: 'POST',
                        data: { paginations:paginations,catname:catname,language:language },
                        success: function(response){
                            var resp1 = $.parseJSON(response);
							console.log(resp1.status+"catname="+catname);
                            if(resp1.status==1){
                            $('.'+catname+paginations).html(resp1.product_details)
                            }
                            if(resp1.nextrec=='0'){
                                $('.www'+catname).addClass('kotaktv_d_none')
                            }
                           // var resp1 = $.parseJSON(response);
