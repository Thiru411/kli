...
class Ulip_plans extends CI_Controller {
	...
	public function index(){
		...
		array_push($this->data['JS_FILES'], 'assets/js/insurance-guide.js', 'assets/js/social-share.js');
			...
			$this->template->load('template/template', 'ulip_plans/ulip-plan', $this->data);
...
$this->data['writters'] = $this->insurance_guide_model->get_reviewdetails('Normal Blog');
	...
	public function what_is_ulip_plan(){
		$data=$this->common_method('wealth-creation','what-is-ulip');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $data);
	}
	
	public function how_ulip_works(){
		$data=$this->common_method('retirement','how-does-ulip-work-in-india');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $data);
	}
	...
	public function ulip_nav(){
		$data=$this->common_method('wealth-creation','how-is-ulip-nav-calculated-how-is-the-return-calculated-in-ulip');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $data);
	}
	
	public function best_investment_options_for_females()
    {
		$data=$this->common_method('wealth-creation','best-investment-options-for-females');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $data);
	}
    ...
    { 
		$this->data=$this->common_method('wealth-creation','how-is-cagr-calculated-in-ulip');

		//Load the view file
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
	
	
public function importance_of_portfolio_management()
    { 
          $this->data=$this->common_method('wealth-creation','importance-of-portfolio-management');
		//Load the view file
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
...
public function minimum_lock_in_period_for_ulip()
    { 
         $data=$this->common_method('retirement','what-is-the-minimum-lock-in-period-for-ulip');
		//Load the view file
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
...
public function benefits_of_ulip()
    { 
        $data=$this->common_method('retirement','benefits-of-ulip');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
...
public function sum_assured_in_unit_linked_insurance_plan()
    ...
    { 
        $data=$this->common_method('protection','ulip-returns-in-15-years');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }

	public function ulip_returns_in_5_years()
    { 
        $data=$this->common_method('protection','ulip-returns-in-5-years');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
	public function ulip_returns_in_20_years()
    { 
        $data=$this->common_method('protection','ulip-returns-in-20-years');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
	public function ulip_returns_in_10_years()
    { 
        $data=$this->common_method('protection','ulip-returns-in-10-years');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
    ...
    { 
        $data=$this->common_method('protection','ulip-for-child-education');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
    ...
    { 
        $data=$this->common_method('protection','ulip-for-retirement');
		$this->template->load('template/template', 'ulip_plans/what_is_ulip_plan', $this->data);
    }
    public function common_method_plans($categorySlug,$urlSlug,$planType){
	  ...
	  $categorySlug = secure_input_data($categorySlug);
		...
		if(strlen($urlSlug) > 0 && $urlSlug != "") {
			...
			if(isset($planMainInfo->entry_id) && isset($planTypeInfo->entry_id) && $planTypeInfo->entry_id > 0 && $categoryURLSlug == $planTypeInfo->plan_type_slug && $planTypeInfo->plan_main_type == $planType) {
				...
				$this->data['mainPlanURL'] = ULIP_PLAN_LINK; //used for similar plan URL
    ...
    { 
		...
		$this->template->load('template/template', 'ulip_plans/kotak_invest_maxima', $data);
    }
